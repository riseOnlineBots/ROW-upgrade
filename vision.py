...
class Vision:
    ...
    upgradable_item_img = None
    upgradable_item_w = 0
    upgradable_item_h = 0

    confirm_button_img = None
    confirm_button_w = 0
    confirm_button_h = 0
    ...
    # constructor
    def __init__(self, upgrade_scroll_path, upgradable_item_path, confirm_button_path, method=cv2.TM_CCOEFF_NORMED):
        ...
        if confirm_button_path:
            # load the image we're trying to match
            # https://docs.opencv2.org/4.2.0/d4/da8/group__imgcodecs.html
            self.confirm_button_img = cv2.imread(confirm_button_path, cv2.IMREAD_UNCHANGED)

            # Save the dimensions of the needle image
            self.confirm_button_w = self.confirm_button_img.shape[1]
            self.confirm_button_h = self.confirm_button_img.shape[0]

        if upgradable_item_path:
            self.upgradable_item_img = cv2.imread(upgradable_item_path, cv2.IMREAD_UNCHANGED)

            # Save the dimensions of the needle image
            self.upgradable_item_w = self.upgradable_item_img.shape[1]
            self.upgradable_item_h = self.upgradable_item_img.shape[0]

        # There are 6 methods to choose from:
        # TM_CCOEFF, TM_CCOEFF_NORMED, TM_CCORR, TM_CCORR_NORMED, TM_SQDIFF, TM_SQDIFF_NORMED
        self.method = method

    def findConfirmButton(self, screenshot, threshold):
        # run the OpenCV algorithm
        result = cv2.matchTemplate(screenshot, self.confirm_button_img, self.method)
        ...
        for loc in locations:
            rect = [int(loc[0]), int(loc[1]), self.confirm_button_w, self.confirm_button_h]
    ...
    def find(self, screenshot, threshold=0.5, max_results=23):
        # run the OpenCV algorithm
        result = cv2.matchTemplate(screenshot, self.upgradable_item_img, self.method)
        ...
        rectangles = []
        for loc in locations:
            rect = [int(loc[0]), int(loc[1]), self.upgradable_item_w, self.upgradable_item_h]
            # Add every box to the list twice in order to retain single (non-overlapping) boxes
            rectangles.append(rect)
            rectangles.append(rect)
        # Apply group rectangles.
    ...
    def match_keypoints(self, original_image, patch_size=32):
        ...
        keypoints_needle, descriptors_needle = orb.detectAndCompute(self.upgrade_scroll_img, None)
